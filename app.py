...
if page == "Gyakorlás":
    ...
    if not st.session_state["practice_answered"]:
        if multiple_choice:
            ...
            answer = st.selectbox("Fajnév:", [""] + all_species, index=0, key="practice_input", placeholder="Kezdj el gépelni...", accept_new_options=True)
    ...
    if st.session_state["exam_images"] is None:
        ...
        with st.form(f"exam_form_{idx}"):
            if multiple_choice:
                ...
                species_answer = st.selectbox("Fajnév:", [""] + all_species, index=0, key=f"exam_input_{idx}", placeholder="Kezdj el gépelni...", accept_new_options=True)
    ...
    if not st.session_state["trophy_answered"]:
        with st.form("trophy_form"):
            if multiple_choice:
                ...
                trophy_species_list = sorted(set(e["species"] for e in trophy_data_list))
                species_answer = st.selectbox("Fajnév:", [""] + trophy_species_list, index=0, key="trophy_species_input", placeholder="Kezdj el gépelni...", accept_new_options=True)
...
# ============================================================
    ...
    with col2:
        group_filter = st.selectbox("Csoport:", ["Mind", "Madár", "Emlős"])
    ...
    if group_filter == "Madár":
        filtered = filter_by_group(filtered, "madár")
    elif group_filter == "Emlős":
        filtered = filter_by_group(filtered, "emlős")
